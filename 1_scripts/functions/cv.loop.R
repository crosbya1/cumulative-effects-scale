...
cv.loop <- function(blocks, blocks_cov, points, points_cov, spp, det, nsurv, k, ni, nt, nb, nc){
  ...
  score <- sapply(1:k, function(j){
    ...
    data <- list(ntrain.block = nrow(b.t), ntrain.site = nrow(pts.t), ntest.block = length(test), 
                 ...
                 point.block.test = as.numeric(as.factor(pts.test$block)), indicator = zst)
    
    system.time({
      out <- jags(data = data, inits = inits, parameters.to.save =  params_1, model.file =  "1_scripts/model_scripts/block_occupancy_1_cv.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
    })
...
cv.loop(blocks = block.test, blocks_cov = block_cov.test, points = pt.test, points_cov = point_cov, spp = spp_pt, det = det, 
        nsurv = bg_nsurv_veg, k = 5, ni = 1000, nt = 1, nb = 500, nc = 3)
